...
app.delete('/api/admin/enquiries/:id', (req, res) => {
  try { deleteEnquiry(req.params.id); res.json({ ok: true }); }
...
app.post('/api/admin/enquiries/delete', (req, res) => {
  ...
  try { deleteEnquiry(id); res.json({ ok: true }); }
  catch { res.status(400).json({ error: 'Failed to delete' }); }
});
...
app.delete('/api/admin/searches/:id', (req, res) => {
  try { deleteSearch(req.params.id); res.json({ ok: true }); }
  catch { res.status(400).json({ error: 'Failed to delete' }); }
});
...
app.post('/api/admin/searches/delete', (req, res) => {
  const id = req.body?.id;
  if (!id) return res.status(400).json({ error: 'id required' });
  try { deleteSearch(id); res.json({ ok: true }); }
  catch { res.status(400).json({ error: 'Failed to delete' }); }
});
